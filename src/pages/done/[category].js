...
import { getDoneByCategory, getTodosByCategory } from "@/modules/Data"
...
export default function TodosByCategory() {
    ...
    const fetchData = useCallback(async () => {
        ...
        const data = await getDoneByCategory(token, userId, category)
    ...
    return (<>
        ...
        <Link href={`/todos/${category}`}>View Todo items in {category}</Link>
        <div
            css={css`
                display: flex;
                flex-direction: row;
